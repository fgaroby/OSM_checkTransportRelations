...
/**
 * A Set-like class that allows looking up equivalent preexising instance. It is
 * useful whereever one would use self-mapping construct like
...
public class Storage<T> extends AbstractSet<T>
{
	 ...
	 * @param capacity
	 * @param safeIterator
	 *            If set to false, you must not modify the Storage while
	 *            iterating over it. If set to true, you can savely modify, but
	 *            the read-only iteration will happen on a copy of the
	 *            unmodified Storage. This is similar to CopyOnWriteArrayList.
	 */
	public Storage( Hash<? super T, ? super T> ha, int capacity,
	        boolean safeIterator )
	{
		this.hash = ha;
		int cap = 1 << ( int ) ( Math.ceil( Math.log( capacity / loadFactor )
		        / Math.log( 2 ) ) );
	...
	private <K> int getBucket( Hash<K, ? super T> ha, K key )
	{
		...
		{
			if ( ha.equals( key, entry ) ) return bucket;
	...
	/*
	 * public static <O> Hash<O,O> identityHash() { return new Hash<O,O>() {
	 * public int getHashCode(O t) { return System.identityHashCode(t); } public
	 * boolean equals(O t1, O t2) { return t1 == t2; } }; }
	 */
