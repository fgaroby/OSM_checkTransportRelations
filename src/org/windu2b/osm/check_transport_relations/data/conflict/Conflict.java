 ...
 * a pair of {@link OsmPrimitive}s where one element of the pair has the role
 * <em>my</em> and the other has the role <em>their</em>.
 * <ul>
 * <li><code>my</code> is the {@link OsmPrimitive} in the local dataset</li>
 * <li><code>their</code> is the {@link OsmPrimitive} which caused the conflict
 * when it it was tried to merge it onto <code>my</code>. <code>their</code> is
 * usually the {@link OsmPrimitive} from the dataset in another layer or the one
...
public class Conflict<T extends OsmPrimitive>
{
	...
	/**
	 * Replies true if the primitive <code>primitive</code> is participating in
	 * this conflict
	 * 
	 * @param primitive
	 *            the primitive
	 * @return true if the primitive <code>primitive</code> is participating in
	 *         this conflict
	 */
	...
	/**
	 * Replies true if the primitive with id <code>id</code> is participating in
	 * this conflict
	 * 
	 * @param id
	 *            the primitive id
	...
	public boolean equals( Object obj )
	{
		...
		if( their != other.their ) return false;
		return true;
	}
	...
	/**
	 * 
	 * @return True if my primitive was deleted but it has set non deleted
	 *         status because it's referred by another primitive and references
	 *         to deleted primitives are not allowed.
	 */
