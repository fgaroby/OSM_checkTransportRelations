...
/**
 * Exception thrown when a communication error occurs when accessing the <a
 * href="http://wiki.openstreetmap.org/wiki/API_v0.6">OSM API</a>.
 * 
 * @see OsmApi
 */
public class OsmApiException extends OsmTransferException
{
	...
	/**
	 * Constructs an {@code OsmApiException} with the specified response code,
	 * error header and error body
	 * 
	 * @param responseCode
	 *            The HTTP response code replied by the OSM server. See
	 *            {@link java.net.HttpURLConnection HttpURLConnection} for
	 *            predefined HTTP response code values
	 * @param errorHeader
	 *            The error header, as transmitted in the {@code Error} field of
	 *            the HTTP response header
	 * @param errorBody
	 *            The error body, as transmitted in the HTTP response body
	 */
	public OsmApiException( int responseCode, String errorHeader,
	        String errorBody )
	{
	...
	/**
	 * Constructs an {@code OsmApiException} with the specified cause and a
	 ...
	 * 
	 * @return The HTTP response code replied by the OSM server. Refer to <a
	 *         href="http://wiki.openstreetmap.org/wiki/API_v0.6">OSM API</a> to
	 *         see the list of response codes returned by the API for each call.
	 */
	 ...
	 * 
	 * @return the error header, as transmitted in the {@code Error} field of
	 *         the HTTP response header
	 */
	...
	public String getMessage()
	{
		...
		try
		{
			if ( errorHeader != null ) eh = tr( errorHeader.trim() );
			if ( !eh.isEmpty() )
			{
	...
	public String getDisplayMessage()
	{
		...
		{
			sb.append( tr( "The server replied an error with code {0}.",
			        responseCode ) );
		}
		return sb.toString();
	}
	...
	/**
	 * Sets the complete URL accessed when this error occured. This is distinct
	 * from the one set with {@link #setUrl}, which is generally only the base
	 * URL of the server.
	 * 
	 * @param url
	...
	/**
	 * Replies the complete URL accessed when this error occured. This is
	 * distinct from the one returned by {@link #getUrl}, which is generally
	 * only the base URL of the server.
	 * 
