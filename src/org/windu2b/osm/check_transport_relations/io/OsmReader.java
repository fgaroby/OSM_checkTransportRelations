...
import org.windu2b.osm.check_transport_relations.data.osm.NodeData;
...
import org.windu2b.osm.check_transport_relations.data.osm.RelationData;
import org.windu2b.osm.check_transport_relations.data.osm.RelationMemberData;
import org.windu2b.osm.check_transport_relations.data.osm.Tagged;
import org.windu2b.osm.check_transport_relations.data.osm.Way;
import org.windu2b.osm.check_transport_relations.data.osm.WayData;
...
import org.windu2b.osm.check_transport_relations.tools.DateUtils;

/**
 * Parser for the Osm Api. Read from an input stream and construct a dataset out
 * of it.
 * 
...
public class OsmReader extends AbstractReader
{
	...
	private long parseWayNode( Way w ) throws XMLStreamException
	{
		...
		{
			throwException( tr(
			        "Missing mandatory attribute ''{0}'' on <nd> of way {1}.",
			        "ref", w.getUniqueId() ) );
		}
		long id = getLong( "ref" );
		if ( id == 0 )
		{
			throwException( tr(
			        "Illegal value of attribute ''ref'' of element <nd>. Got {0}.",
			        id ) );
		}
		jumpToEnd();
		return id;
	        ...
	        throws XMLStreamException
	{
		...
		{
			throwException( tr(
			        "Missing attribute ''ref'' on member in relation {0}.",
			        r.getUniqueId() ) );
		}
		try
		{
	...
	/**
	 * When cursor is at the start of an element, moves it to the end tag of
	 * that element. Nested content is skipped.
	 * 
	 * This is basically the same code as parseUnknown(), except for the
	 * warnings, which are displayed for inner elements and not at top level.
	 */
	...
	private void readCommon( PrimitiveData current ) throws XMLStreamException
	{
		...
		if ( versionString != null )
		{
			...
			if ( ds.getVersion().equals( "0.6" ) )
			{
				if ( version <= 0 && current.getUniqueId() > 0 )
				{
					throwException( tr(
					        "Illegal value for attribute ''version'' on OSM primitive with ID {0}. Got {1}.",
					        Long.toString( current.getUniqueId() ),
					        versionString ) );
				}
				else if ( version < 0 && current.getUniqueId() <= 0 )
				{
					System.out
					        .println( tr(
					                "WARNING: Normalizing value of attribute ''version'' of element {0} to {2}, API version is ''{3}''. Got {1}.",
			...
			else if ( ds.getVersion().equals( "0.5" ) )
			{
				if ( version <= 0 && current.getUniqueId() > 0 )
				{
					System.out
					        .println( tr(
					                "WARNING: Normalizing value of attribute ''version'' of element {0} to {2}, API version is ''{3}''. Got {1}.",
					                current.getUniqueId(), version, 1, "0.5" ) );
					version = 1;
				}
				else if ( version < 0 && current.getUniqueId() <= 0 )
				{
					System.out
					        .println( tr(
					                "WARNING: Normalizing value of attribute ''version'' of element {0} to {2}, API version is ''{3}''. Got {1}.",
					                current.getUniqueId(), version, 0, "0.5" ) );
					version = 0;
				}
			}
			else
		...
		{
			// version expected for OSM primitives with an id assigned by the
			// server (id > 0), since API 0.6
			//
			if ( current.getUniqueId() > 0 && ds.getVersion() != null
			        && ds.getVersion().equals( "0.6" ) )
			{
				throwException( tr(
				        "Missing attribute ''version'' on OSM primitive with ID {0}.",
				        Long.toString( current.getUniqueId() ) ) );
			}
			else if ( current.getUniqueId() > 0 && ds.getVersion() != null
			        && ds.getVersion().equals( "0.5" ) )
			{
				...
				version = 1;
			        ...
			        && ds.getVersion().equals( "0.5" ) )
			{
				// default version in 0.5 files for new primitives, no warning
				// necessary. This is
	...
	private long getLong( String name ) throws XMLStreamException
	{
		...
		{
			throwException( tr(
			        "Illegal long value for attribute ''{0}''. Got ''{1}''.",
			        name, value ) );
		}
	...
	private static class OsmParsingException extends XMLStreamException
	{
		...
		{
			super( msg ); /*
						 * cannot use super(msg, location) because it messes
						 * with the message preventing localization
						 */
		...
		public String getMessage()
		{
			...
			if ( getLocation() == null ) return msg;
			msg = msg
			        + " "
			        + tr( "(at line {0}, column {1})", getLocation()
			                .getLineNumber(), getLocation().getColumnNumber() );
			return msg;
		}
	}
	        ...
	        ProgressMonitor progressMonitor ) throws IllegalDataException
	{
		...
		try
		{
			...
			InputStreamReader ir = UTFInputStreamReader
			        .create( source, "UTF-8" );
			XMLStreamReader parser = XMLInputFactory.newInstance()
			        .createXMLStreamReader( ir );
			setParser( parser );
			parse();
			progressMonitor.worked( 1 );

			progressMonitor
			        .indeterminateSubTask( tr( "Preparing data set..." ) );
			prepareDataSet();
			progressMonitor.worked( 1 );
		...
		catch ( XMLStreamException e )
		{
			...
			if ( e.getLocation() != null )
				throw new IllegalDataException( tr( "Line {0} column {1}: ", e
	 ...
	 *             thrown if the an error was found while parsing the data from
	 *             the source
