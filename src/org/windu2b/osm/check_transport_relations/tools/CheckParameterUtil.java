 ...
 * checking parameters at run-time.
 * 
 */
public class CheckParameterUtil
{
	        ...
	        String parameterName ) throws IllegalArgumentException
	{
		...
		if ( id.getUniqueId() <= 0 )
		    throw new IllegalArgumentException( MessageFormat.format(
		            "Expected unique id > 0 for primitive ''{1}'', got {0}",
		            id.getUniqueId(), parameterName ) );
	}
	        ...
	        throws IllegalArgumentException
	{
		if ( version < 0 )
		    throw new IllegalArgumentException(
		            MessageFormat
		                    .format(
		                            "Expected value of type long > 0 for parameter ''{0}'', got {1}",
	        ...
	        String parameterName )
	{
		if ( value == null )
		    throw new IllegalArgumentException( MessageFormat.format(
		            "Parameter ''{0}'' must not be null", parameterName ) );
	}
	...
	/**
	 * Ensures that <code>id</code> is non-null primitive id of type
	 * {@link OsmPrimitiveType#NODE}
	 * 
	 * @param id
	 *            the primitive id
	 * @param parameterName
	        ...
	        throws IllegalArgumentException
	{
		...
		if ( !id.getType().equals( OsmPrimitiveType.NODE ) )
		    throw new IllegalArgumentException( MessageFormat.format(
		            "Parameter ''{0}'' of type node expected, got ''{1}''",
		            parameterName, id.getType().getAPIName() ) );
	}
}
