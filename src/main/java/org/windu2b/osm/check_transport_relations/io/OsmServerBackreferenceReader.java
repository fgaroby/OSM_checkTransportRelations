...
import org.windu2b.osm.check_transport_relations.data.osm.DataSetMerger;
 ...
 * refer to a specific primitive. For a {@link Node}, ways and relations are
 * retrieved which refer to the node. For a {@link Way} or a {@link Relation},
 * only relations are read.
 * 
 * OsmServerBackreferenceReader uses the API calls
 * <code>[node|way|relation]/#id/relations</code> and <code>node/#id/ways</code>
 * to retrieve the referring primitives. The default behaviour of these calls is
 * to reply incomplete primitives only.
 * 
 * If you set {@link #setReadFull(boolean)} to true this reader uses a
 * {@link MultiFetchServerObjectReader} to complete incomplete primitives.
 * 
 * 
 */
public class OsmServerBackreferenceReader extends OsmServerReader
{
	 ...
	 * 
	 * @param primitive
	 *            the primitive to be read. Must not be null. primitive.id > 0
	 *            expected
	 * 
	 * @exception IllegalArgumentException
	 *                thrown if primitive is null
	 * @exception IllegalArgumentException
	 *                thrown if primitive.id <= 0
	 */
	 ...
	 * 
	 * @param id
	 *            the id of the primitive. > 0 expected
	 * @param type
	        ...
	        throws IllegalArgumentException
	{
		if( id <= 0 )
		    throw new IllegalArgumentException( MessageFormat.format(
		            "Parameter ''{0}'' > 0 expected. Got ''{1}''.", "id", id ) );
	 ...
	 *            the id of the primitive. > 0 expected
	 * @param readFull
	 *            true, if referers should be read fully (i.e. including their
	 *            immediate children)
	 * 
	 */
	public OsmServerBackreferenceReader( OsmPrimitive primitive,
	 ...
	 * 
	 * @param primitive
	 *            the primitive whose referers are to be read
	 * @param readFull
	 *            true, if referers should be read fully (i.e. including their
	 *            immediate children)
	 * 
	 ...
	 */
	public OsmServerBackreferenceReader( long id, OsmPrimitiveType type,
	        boolean readFull ) throws IllegalArgumentException
	{
		this( id, type );
		this.readFull = false;
	}
	...
	/**
	 * Replies true if this reader also reads immediate children of referring
	 * primitives
	 * 
	 * @return true if this reader also reads immediate children of referring
	 *         primitives
	 */
	...
	/**
	 * Set true if this reader should reads immediate children of referring
	 * primitives too. False, otherweise.
	 * 
	 * @param readFull
	 *            true if this reader should reads immediate children of
	 *            referring primitives too. False, otherweise.
	 */
	        ...
	        throws OsmTransferException
	{
		...
		{
			progressMonitor
			        .indeterminateSubTask( tr( "Downloading from OSM Server..." ) );
			StringBuffer sb = new StringBuffer();
			sb.append( primitiveType.getAPIName() ).append( "/" ).append( id )
			        .append( "/ways" );

			in = getInputStream( sb.toString(),
			        progressMonitor.createSubTaskMonitor( 1, true ) );
			if( in == null ) return null;
			progressMonitor.subTask( tr( "Downloading referring ways ..." ) );
			return OsmReader.parseDataSet( in,
			        progressMonitor.createSubTaskMonitor( 1, true ) );
		}
	        ...
	        throws OsmTransferException
	{
		...
		try
		{
			...
			if( in == null ) return null;
			progressMonitor
			        .subTask( tr( "Downloading referring relations ..." ) );
			return OsmReader.parseDataSet( in,
			        progressMonitor.createSubTaskMonitor( 1, true ) );
		}
		catch( OsmTransferException e )
	...
	/**
	 * Scans a dataset for incomplete primitives. Depending on the configuration
	 * of this reader incomplete primitives are read from the server with an
	 * individual <tt>/api/0.6/[way,relation]/#id/full</tt> request.
	 * 
	 * <ul>
	 * <li>if this reader reads referers for a {@link Node}, referring ways are
	 * always read individually from the server</li>
	 * <li>if this reader reads referers for an {@link Way} or a
	 * {@link Relation}, referring relations are only read fully if
	 * {@link #setReadFull(boolean)} is set to true.</li>
	 * </ul>
	 * 
	        ...
	        ProgressMonitor progressMonitor ) throws OsmTransferException
	{
		...
		try
		{
				...
				for( Way way : waysToCheck )
				{
					if( way.hasIncompleteNodes() )
					{
						OsmServerObjectReader reader = new OsmServerObjectReader(
						        way.getId(), OsmPrimitiveType.from( way ), true /*
																				 * read
																				 * full
																				 */);
						DataSet wayDs = reader.parseOsm( progressMonitor
			...
			if( isReadFull() )
			{
				Collection<Relation> relationsToCheck = new ArrayList<Relation>(
				        ds.getRelations() );
				for( Relation relation : relationsToCheck )
				{
					if( relation.hasIncompleteMembers() )
					{
						OsmServerObjectReader reader = new OsmServerObjectReader(
						        relation.getId(),
						        OsmPrimitiveType.from( relation ), true /*
																		 * read
																		 * full
																		 */);
						DataSet wayDs = reader.parseOsm( progressMonitor
	 ...
	 * 
	 * @param progressMonitor
	 *            the progress monitor. Set to
	 *            {@link NullProgressMonitor#INSTANCE} if null.
	 * @return the dataset with the referring primitives
	 * @exception OsmTransferException
	 *                thrown if an error occurs while communicating with the
	 *                server
	 */
	@Override
