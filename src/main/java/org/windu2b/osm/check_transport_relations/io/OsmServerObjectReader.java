 ...
 * 
 * It can either download the object including or not including its immediate
 * children. The former case is called a "full download".
 * 
 * It can also download a specific version of the object (however, "full"
 * download is not possible in that case).
 * 
 */
public class OsmServerObjectReader extends OsmServerReader
{
	...
	/**
	 * true if a full download is required, i.e. a download including the
	 * immediate children
	 */
	...
	/**
	 * the specific version number, if required (incompatible with full), or -1
	 * else
	 */
	private int	        version;
	 ...
	 * 
	 * @param id
	 *            the object id. > 0 required.
	 * @param type
	 *            the type. Must not be null.
	 * @param full
	 *            true, if a full download is requested (i.e. a download
	 *            including immediate children); false, otherwise
	...
	protected OsmServerObjectReader( long id, OsmPrimitiveType type,
	        boolean full, int version ) throws IllegalArgumentException
	{
		if ( id <= 0 )
		    throw new IllegalArgumentException( MessageFormat.format(
		            "Expected value > 0 for parameter ''{0}'', got {1}", "id",
		            id ) );
	 ...
	 * 
	 * @param id
	 *            the object id. Must not be null. Unique id > 0 required.
	 * @param full
	 *            true, if a full download is requested (i.e. a download
	 *            including immediate children); false, otherwise
	 * @throws IllegalArgumentException
	        ...
	        throws OsmTransferException
	{
		...
		{
			progressMonitor
			        .indeterminateSubTask( tr( "Downloading OSM data..." ) );
			...
			final DataSet data = OsmReader.parseDataSet( in, progressMonitor
