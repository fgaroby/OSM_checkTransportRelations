...
public class SimplePrimitiveId implements PrimitiveId, Serializable
{
	 ...
	 * 
	 * @param s
	 *            the string to be parsed, e.g., {@code n1}, {@code node1},
	 *            {@code w1}, {@code way1}, {@code r1}, {@code rel1},
	 *            {@code relation1}.
	...
	public static SimplePrimitiveId fromString( String s )
	{
		final Pattern p = Pattern
		        .compile( "((n(ode)?|w(ay)?|r(el(ation)?)?)/?)(\\d+)" );
		final Matcher m = p.matcher( s );
		if ( m.matches() )
		{
			return new SimplePrimitiveId( Long.parseLong( m.group( m
			        .groupCount() ) ),
