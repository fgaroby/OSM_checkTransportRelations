...
public class Base64
{
	...
	public static String encode( String s, boolean urlsafe )
	{
		...
		for ( int i = 0; i < ( s.length() + 2 ) / 3; ++i )
		{
			...
			out.append( enc.charAt( ( buf.charAt( 0 ) & 0x03 ) << 4
			        | ( l == 1 ? 0 : ( buf.charAt( 1 ) & 0xf0 ) >> 4 ) ) );
			out.append( l > 1 ? enc.charAt( ( buf.charAt( 1 ) & 0x0f ) << 2
	...
	public static String encode( ByteBuffer s, boolean urlsafe )
	{
		...
		for ( int i = 0; i < ( s.limit() + 2 ) / 3; ++i )
		{
			...
			out.append( enc.charAt( ( byte0 & 0x03 ) << 4
			        | ( l == 1 ? 0 : ( byte1 & 0xf0 ) >> 4 ) ) );
			out.append( l > 1 ? enc.charAt( ( byte1 & 0x0f ) << 2
			        | ( l == 2 ? 0 : ( byte2 & 0xc0 ) >> 6 ) ) : '=' );
